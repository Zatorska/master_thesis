...
\todo[inline]{Dodać cytowania dotyczące modeli ARIMA i SARIMA, np. typowe podręczniki. }
...
\todo[inline]{W poniższym akapicie musi być napisane, że parametry są dobierane tylko na podstawie danego okresu czasu. W szczególności nie chcemy pisać o tym, że funkcja dzieli zbiory na treningowy i testowy. Poniższą całość proszę jeszcze przeformułować. }
...
\todo[inline]{Podać tylko wyniki bez opisu i bez wyliczenia. }
...
%  \captionsetup{margin=10pt,font=small,labelfont=bf,width=.8\textwidth}
%
%  \caption[Najlepsze parametry do modelu SARIMA, wybrane za pomocą funkcji \code{sarima\_split\_best\_params\_search\_fit\_predict\_plot}.]{Najlepsze parametry do modelu SARIMA, wybrane za pomocą funkcji  \code{sarima\_split\_best\_params\_search\_fit\_predict\_plot}. \textit{Źródło:} opracowanie własne.}
%  \label{tab:tabela8}
%  
%  \footnotesize
%
%\vspace*{2ex}
%
%\input{.//out\_tables//tab\_08.tex}
%
%\end{table}

\subsection{TBATS}

\todo[inline]{Dodać cytowania związane z tym modele.}
...
W modelu użyto również stałego parametru \code{s = 52}, który odpowiada liczbie okresów w sezonie, w tym przypadku reprezentującej liczbę tygodni w roku.

\todo[inline]{(1) Wyrzucić kolumnę dotyczącą liczby core'ów procesora. (2) Jeżeli nadal się nie mieści to obrócić nagłówki do pionu.}
...
\todo[inline]{Dokładniej opisać tę topologię. Ewentualnie dać tutaj kawałek kodu definiującego tę topologię. }
...
\todo[inline]{(1) Dla każdego kraju dzielimy rzeczywiste zapotrzebowanie przez prognozę (czyli prognoza jest naszym punktem odniesienia). Zapisujemy to jako procenty \(100 \cdot (1 - y/\hat{y})\). (2) Dla Polski robimy z tego wykres w okresie testowym. (3) Dla każdego kraju i każdego modelu, obliczamy minimum (jeżeli spadki już w procentach to wtedy maksimum) i z tego robimy tabelkę. (3) Dyskusja zawiera dwa aspekty: (a) w zależności od kraju i modelu oszacowanie spadku istotnie się różni. (b) następuje szybko powrót do poziomów popytu z przed pandemii (a więc spadki zbliżają się do zera). Zgodnie z tym zmieniamy poniższą dyskusję. }
...
\todo[inline]{Celem pracy jest pokazanie jak bardzo różnią się oszacowania spadków zapotrzebowania na moc w czasie pandemii w zależności od wykorzystanego modelu prognostycznego.}

%Wartości z tabeli~\ref{tab:tabela12} potwierdzają wnioski wysnute podczas analizy wykresu~\ref{fig:x13} --- najgorsza skuteczność modelu LSTM i porównywalne wyniki dla pozostałych modeli, z nieznaczną korzyścią dla TBATS. 
...
%  \captionsetup{margin=10pt,font=small,labelfont=bf,width=.8\textwidth}
%
%  \caption[Wyniki MAPE dla modeli SARIMA, TBATS i LSTM dla każdego z badanych krajów.]{Wyniki MAPE dla modeli SARIMA, TBATS i LSTM dla każdego z badanych krajów. \textit{Źródło:} opracowanie własne.}
%  \label{tab:tabela12}
%  
%  \footnotesize
%
...
\todo[inline]{Tutaj na końcu wpisujemy podsumowanie przeprowadzonego badania i podstawowych wyników.}
...
Głównym celem niniejszej pracy jest analiza wpływu wyboru modelu prognostycznego na dokładność oszacowania skutków pandemii COVID-19. Badanie zostało przeprowadzone na danych Europejskiego Systemu Operatorów Przesyłowych Energii Elektrycznej (ENTSOE) obejmujących 23 kraje Unii Europejskiej. Dla każdego z tych krajów zastosowano trzy różne modele prognostyczne: SARIMA, TBATS oraz LSTM.

Podstawą do analizy było użycie danych sprzed pandemii, sięgających początku 2015 roku, w celu prognozowania zużycia energii elektrycznej na okres od 2020 roku. Badanie obejmowało szeroki zakres aspektów, od technicznych detalii zastosowanych modeli po analizę wyników prognoz.

\todo[inline]{Dokończyć - trzeba dopisać wnioski}
