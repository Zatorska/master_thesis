...
\todo[inline]{W końcu nie jestem pewna co we Wprowadzeniu zrobić z subsubsections: mam zostawić jak jest (nie są widoczne w spisie treści), usunąć całkowicie, czy po prostu usunąć framgent 'subsubsection' zostawiając sam podtytuł?}
...
\todo[inline]{Czy powinnam dodatkowo zformatować tytuły rozdziałów, np. pogrubieniem/zrobić odnośniki?}
...
\todo[inline]{Zastanawiam się czy ten wstęp jest ok? Nie za bardzo powtarza informacje ze Struktury Pracy?}
...
W tabeli~\ref{tab:tabela3} przedstawiono przykładowe dane uzyskane w wyniku przeprowadzonych operacji, natomiast na wykresie~\ref{fig:x11} zaprezentowano odpowiadający tym danym szereg czasowy.
...
Zbiór danych treningowych obejmował zakres dat od 2015-01-11 do 2019-12-29 (z wyłączeniem Cypru, dla którego zbiór treningowy zaczynał się od 2016-09-21 ze względu na znaczną ilość brakujących danych).
...
\todo[inline]{Pokazujemy uzyskane wyniki (a więc oszacowane spadki zużycia energii elektrycznej) oraz różnice w tych oszacowania dla różnych modeli. Na tej podstawie dyskutujemy różnice ale również czy wpływają one na np. zmianę kolejności uporządkowania krajów jeżeli chodzi o wielkości ograniczenia zużycia. Idea jest taka aby zobaczyć, czy różnice wprowadzają zmiany jakościowe. --- o to chciałabym się jeszcze raz dopytać}
